...
let hypot = defn({
  name: 'hypot',
  doc: 'Returns the "hypoteneuse" of a list of numbers: the square root of the sum of their squares. This will calculate the distance between the origin and  Note that this can be slow, and to compare, e.g. the magnitude of vectors, you should probably use `sum_of_squares`.',
  pre: splat(is_number),
...
let Math_ = ns({
  name: 'Math',
  space: {
    abs, add, ceil, div, div_by, floor, gt, gte, hypot,
    is_natural, is_negative, is_nonzero, is_positive, is_positive_int,
    lt, lte, mod, mult, number, pow, pow_by, precise, random, random_int, round,
    sqrt, sub, sub_by, sum_of_squares, trunc,
    clamp, lerp, norm, rad_to_deg, deg_to_rad,
    cos, sin, tan, acos, asin, atan, atan2, ln, log2, log10,
