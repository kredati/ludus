...
let {defn} = Fn;
let {args, int, any, fn, or, coll} = Spec;
let {seq, is_empty, size, first, rest}= Seq;
let {ns} = NS;
...
let range = defn({
  ...
  pre: args([int], [int, int], [int, int, int]),
  body: [
    (max) => range(0, max, 1),
...
let cycle = defn({
  ...
  pre: args([coll], [or(int, infinite), coll]),
  body: [
    (seqable) => cycle(Infinity, seqable),
    (count, seqable) => {
      let seq_size = size(seqable);
      let cycle_size = P.is_some(seq_size) ? count * seq_size : undefined;
...
let interleave = defn({
  ...
  body: (...seqables) => {
    size(seqables[0]) //?
    let seq_size = Math.min(...seqables.map((s) => size(s))) * size(seqables);
